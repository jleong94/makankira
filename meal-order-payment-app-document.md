...
- Remind the organizer to submit the order before meal time, by email and Web Push, scheduled via a cron job.
...
Generate participant-specific WhatsApp messages.
...
- Cloud file export.
- Scheduled order-submission reminders: a **Vercel Cron** job triggers a server endpoint that sends **email** (via a transactional email service such as Resend) and **Web Push** (VAPID + the `web-push` library) to the organizer before the meal time.
...
- **Order-submission reminders** use `meal_sessions.remind_at` (= `meal_date_time` minus `reminder_lead_minutes`). A Vercel Cron job fires once per session (guarded by `reminder_sent_at`) and only while `status` is `draft` or `collecting_orders`, sending email (`users.email`) and Web Push (`push_subscriptions`).
...
- **`Auth` column below:** `Public` = no session needed; `Owner` = authenticated owner of the meal session; `User` = any authenticated user; `Cron` = invoked only by the Vercel Cron scheduler, authenticated with `CRON_SECRET` (not user-facing).
...
| POST | `/api/cron/reminders` | Cron | Internal: invoked by Vercel Cron (authenticated with `CRON_SECRET`); sends due order-submission reminders by email + Web Push, once per session |
...
| **Method** | **Path** | **Auth** | **Purpose (Screen)** |
| --- | --- | --- | --- |
| GET | `/api/meals/:mealId/payment-requests` | Owner | Localized message per participant, with method details, QR URL, reference, and a `whatsappUrl` (Screen 9) |
...
- Order-submission reminders to the organizer (email + Web Push) via a Vercel Cron job.
...
5. Cron is configured by `vercel.json` (no key). Note: the Hobby plan runs cron jobs on a limited cadence — confirm it meets the reminder timing or upgrade.
...
`vapidSubject` is a non-secret literal set directly in `config/app-config.*.yaml` (no provisioning needed).
